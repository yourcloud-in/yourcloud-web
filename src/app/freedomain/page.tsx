...
const CATEGORY_OPTIONS = [
  ...
  { value: 'ngo'        as Category, icon: '🤝', label: 'NGO / Social Good', desc: 'Non-profit and social impact initiatives' },
];
...
export default function FreeDomainPage() {
  ...
  const fullDomain = form.subdomain ? `${form.subdomain}.yc.in` : 'yourname.yc.in';
      ...
      <div className="min-h-screen bg-[#0A1628] flex items-center justify-center p-6">
        <div className="max-w-md text-center space-y-5">
          ...
          <p className="text-gray-400 text-sm leading-relaxed">
            ...
            under review. We'll email <strong className="text-white">{form.ownerEmail}</strong> within 24 hours.
          </p>
          ...
          <p className="text-xs text-gray-500">
            Questions? <span className="text-[#1D9E75]">ceo@yourcloud.in</span> — Babrit responds personally.
          </p>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#0A1628] text-white">
      <div className="border-b border-white/10 bg-[#0D2137]">
        <div className="max-w-2xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <span className="font-semibold text-sm">YourCloud</span>
      ...
      <div className="max-w-2xl mx-auto px-6 py-10 space-y-8">
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-gray-400 text-sm leading-relaxed max-w-lg mx-auto">
            Get a real, live <strong className="text-white font-mono">yourproject.yc.in</strong> domain.
          ...
          {[1,2,3].map((s, i) => (
            <div key={s} className={`flex items-center ${i < 2 ? 'flex-1' : ''}`}>
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                step > s ? 'bg-[#1D9E75] text-white' : step === s ? 'bg-[#1D9E75] text-white' : 'bg-gray-800 text-gray-500'
              }`}>{step > s ? '✓' : s}</div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <div className="flex-1 flex items-center bg-[#0D2137] border border-gray-700 rounded-xl overflow-hidden focus-within:border-[#1D9E75]">
                  ...
                  <span className="px-4 text-[#1D9E75] font-mono text-sm font-semibold border-l border-gray-700 py-3 bg-[#1D9E75]/5">.yc.in</span>
                </div>
              ...
              {available === true  && <p className="text-xs text-green-400 mt-1">✅ <span className="font-mono">{form.subdomain}.yc.in</span> is available!</p>}
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {[['GitHub Pages','youruser.github.io'],['Vercel','cname.vercel-dns.com'],['Netlify','netlify.app'],['Render','onrender.com']].map(([name,cname]) => (
                  <button key={name} onClick={() => set('targetCname', cname)}
                    className="text-xs text-gray-400 border border-gray-700 px-3 py-1.5 rounded-lg hover:border-[#1D9E75]/50 hover:text-[#1D9E75] transition-colors">
                    {name}
                  </button>
                ))}
              </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div className="bg-[#1D9E75]/5 border border-[#1D9E75]/20 rounded-xl p-4 text-xs text-gray-400 space-y-1">
              <p className="font-semibold text-white mb-1.5">📋 Terms</p>
              <div>✅ Free for 1 year · renewable at no cost</div>
              <div>✅ For educational / non-commercial use only</div>
              <div>✅ Solar-powered DNS · DPDP compliant · data in Odisha</div>
              <div>❌ No spam, phishing, or illegal content</div>
              <div>❌ No reselling or transfer</div>
            </div>
        ...
        <div className="text-center text-xs text-gray-600 pb-4">
          YourCloud Free Domain Initiative · Bhubaneswar, Odisha · ☀️ 100% Solar · 🛡️ DPDP Compliant<br/>
          A helping hand for India's youth · <a href="mailto:ceo@yourcloud.in" className="text-[#1D9E75] hover:underline">ceo@yourcloud.in</a>
        </div>
